...
var handleCompOperator = function(arr) {
    ...
    var op = arr.shift()
    var arrTail = arr.slice(1)

    /* Remove last element from arr to make
       both arr and arrTail of the same length
       so that they can be 'zipped' */
    arr.pop();

    arr.forEach(function (value, index, _) { arr[index] += " " + op + " " + arrTail[index]})

    if (arr.length == 1)
        return "(" + arr[0] + ")";
    else
        arr.unshift('&&')
        return handleLogicalOperator(arr)
}
...
var handleArithOperator = function(arr) {
    ...
    arr.forEach(function (value, _, _) { ret += value + " " + op + " "});
