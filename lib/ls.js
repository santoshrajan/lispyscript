...
var require_ls = function(file) {
    var macro_file = require.resolve("../src/macros.ls")
    if (!cache[macro_file]) {
        compile(fs.readFileSync(macro_file, "utf8"))
        cache[macro_file] = true
    }
    
    try {
        var ls_file = fs.realpathSync(file)
    } catch(err) {
        throw new Error("Cannot find required ls file " + file)
    }
}
