...
var parse = function(code, filename) {
    ...
    var parser = function() {
        ...
        while (pos < length) {
            ...
            if (c == ']') {
                if (isJSArray === 0) handleError(4, tree._line, tree._filename);
                isJSArray--;
                token += c;
                continue;
            }
            ...
            if (c == '}') {
                if (isJSObject === 0) handleError(6, tree._line, tree._filename);
                isJSObject--;
                token += c;
                continue;
            }
...
var handleExpression = function(expr) {
    ...
    var fName = expr[0];
    if (!fName) handleError(1, expr._line);
    ...
    var replaceCode = function(source) {
        ...
        if (isHomoiconicExpr.test(expr_name)) {
            ...
            if (expr_name === "#args-shift") {
                if (!_.isArray(replarray)) handleError(13, tree._line, tree._filename, command);
                var argshift = replarray.shift();
                if (_.isUndefined(argshift)) handleError(12, tree._line, tree._filename, command);
                return argshift;
            }
            if (expr_name === "#args-second") {
                if (!_.isArray(replarray)) handleError(13, tree._line, tree._filename, command);
                var argsecond = replarray.splice(1, 1)[0];
                if (_.isUndefined(argsecond)) handleError(12, tree._line, tree._filename, command);
                return argsecond;
            }
...
keywords["var"] = function(arr) {
    if (!validName.test(arr[1])) handleError(9, arr._line, arr._filename);
